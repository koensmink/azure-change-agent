...
def classify_security(event: dict) -> dict:
    ...
    if hits:
        ...
        event["category"] = guess_category(hay)
        event["impact"] = "Unknown"
        event["recommended_action"] = "Review change; assess effect on policies, identity, endpoints, and logging."
    else:
...
def guess_category(hay: str) -> str:
    ...
    if any(w in hay for w in ["intune", "endpoint", "device", "compliance", "autopilot"]):
        return "Endpoint"
    if any(w in hay for w in ["defender", "xdr", "siem", "soar", "sentinel"]):
        return "Security"
